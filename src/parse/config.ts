...
function readConfigFile(
  ...
  try {
    ...
    return { config: {}, file: "<readdir_error>" }
  }
  for (const basename of files) {
    ...
    const parse = (() => {
      switch (ext) {
        case ".json": {
          return (s: string) => JSON.parse(s)
        }
        case ".toml": {
          return (s: string) => TOML.parse(s)
        }
        default: {
          throw Error(`${file}: Invalid config file extension "${ext}"`)
        }
      }
    })()
...
export function parseConfig<O extends Opts>(cfg: Cfg, opts: O, mock?: Mock["config"]): Vals<O> {
  ...
  for (const [optName, opt] of Object.entries(opts)) {
    ...
    try {
      ...
      err.message = `${file}: Option ${configKey} error: ${(err as Error).message}`
      throw err
    }
  }
